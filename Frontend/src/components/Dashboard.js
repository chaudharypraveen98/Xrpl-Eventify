...
export const Dashboard = (props) => {
  ...
  function showBurnModal(e) {
    setOpenBurnModal({
      show: true,
      idValue: searchResults.results[e.target.id].NFTokenID,
    });
  }
            ...
            <tr id={index}>
              ...
              <td>
                <button
                  id={index}
                  type="submit"
                  onClick={showBurnModal}
                >
                  Burn NFT
                </button>
              </td>
              <td>
                <button type="submit">Create Sell Offer</button>
              </td>
            </tr>
          );
        })}
  ...
  return (
    <div>
      ...
      {openMintModal.show && (
        <MintModal
          closeModal={() => {
            setMintModal({ show: false, idValue: "" });
          }}
        />
      )}
      ...
      <div className="primaryBox">
        <div>
          {search ? (
            ...
            <h2>Enter valid r-address and click search button</h2>
          )}
        </div>
        <div>
        ...
        <div style={{ textAlign: "center" }}>
          {searchResults.results.length > 0 ? (
            ...
            <h4>No NFTs exist for this account.</h4>
          )}
        </div>
      </div>
    </div>
